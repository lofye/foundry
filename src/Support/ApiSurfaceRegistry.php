...
final class ApiSurfaceRegistry
{
     ...
     */
    private function containsGenerateMetaFlag(array $args): bool
    {
        foreach ($args as $arg) {
            if (
                $arg === '--workflow'
                || str_starts_with($arg, '--workflow=')
                || $arg === '--template'
                || str_starts_with($arg, '--template=')
            ) {
                return true;
            }
        }
    ...
    public function cliCommands(): array
    {
        $commands = [
            ...
            $this->cliCommandEntry('plan:replay', 'plan:replay <plan_id> [--strict] [--dry-run]', 'experimental', 'Replay one persisted generate plan artifact with explicit drift handling and validation.'),
            ...
            $this->cliCommandEntry('history', 'history [--kind=<build|quality|trace|profile|comparison|generate>]', 'experimental', 'List persisted build, quality, observability, and generate history entries.'),
            ...
            $this->cliCommandEntry('extension:install', 'extension:install <path-or-name>', 'experimental', 'Alias for `pack install` to preserve legacy extension marketplace command compatibility.'),
