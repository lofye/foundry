...
final class ApiSurfaceRegistry
{
    ...
    public function cliCommands(): array
    {
        $commands = [
            ...
            $this->cliCommandEntry('license status', 'license status', 'experimental', 'Show local license identity and service-access status from the environment or local store.'),
            ...
            $this->cliCommandEntry('explain', 'explain [<target>] [--type=<kind>] [--markdown] [--deep] [--neighbors|--no-neighbors] [--no-diagnostics] [--no-flow] [--diff]', 'experimental', 'Explain a framework, application, or installed pack subject from the compiled graph, projections, diagnostics, docs metadata, and extension registry. When no target is given, Foundry explains the first feature or route deterministically. JSON output includes deterministic confidence data, and `foundry explain --diff` renders the last architectural generate diff.'),
            ...
            $this->cliCommandEntry('generate <intent>', 'generate <intent...> --mode=<new|modify|repair> [--target=<target>] [--dry-run] [--no-verify] [--explain] [--allow-risky] [--allow-pack-install] [--packs=<vendor/pack,...>]', 'experimental', 'Plan and execute explain-driven, pack-aware system changes through deterministic generators, architectural snapshots, explain diffs, verification loops, and explicit plan/outcome confidence signals.'),
            ...
            $this->cliCommandEntry('init', 'init [--example=<blog-api|extensions-migrations>] [--temp]', 'stable', 'Start the deterministic first-run flow, optionally loading a curated onboarding example without prompts.'),
            ...
            $this->cliCommandEntry('examples:list', 'examples:list', 'stable', 'List curated onboarding examples, including their taxonomy, copy mode, and source examples.'),
            $this->cliCommandEntry('examples:load', 'examples:load <blog-api|extensions-migrations> [--temp]', 'stable', 'Load a curated onboarding example into the current directory or a temporary workspace, then run the first architecture walkthrough.'),
        ...
        foreach ([
            ...
            'graph-spec' => ['stable', 'Inspect the canonical node, edge, invariant, compatibility, and artifact-schema graph contract.'],
            'node-types' => ['stable', 'Inspect the canonical registry of graph node types.'],
            'edge-types' => ['stable', 'Inspect the canonical registry of graph edge types.'],
            'subgraph' => ['stable', 'Inspect feature-scoped subgraphs plus ownership, execution, and observability slices.'],
            'graph-integrity' => ['stable', 'Inspect canonical graph integrity diagnostics without the broader build-artifact wrapper.'],
            ...
            $usage = match ($target) {
                ...
                'build', 'pipeline', 'extensions', 'packs', 'compatibility', 'migrations', 'graph-spec', 'node-types', 'edge-types', 'graph-integrity' => 'inspect ' . $target,
                ...
                'subgraph' => 'inspect subgraph <feature>',
        ...
        foreach ([
            ...
            'graph-integrity' => ['stable', 'verify graph-integrity', 'Verify canonical graph node, edge, structural, and compatibility integrity.'],
