...
Master Prompt for Codex: Build Foundry

Build a production-minded, reusable, LLM-first PHP 8.5+ web framework named Foundry.

Foundry is designed primarily for LLMs creating, inspecting, modifying, and maintaining application code, not for maximizing human developer ergonomics.
...
Aim for extremely high test coverage across the framework core, because Foundry is intended to be reused as infrastructure.
Target:
...
Application structure for apps built with Foundry

Applications built with Foundry must use this structure:
...
Rules:
	...
	•	app/generated/* is generated by Foundry and can be regenerated
...
Rules:
	...
	•	Foundry parses file into a query registry
...
interface CacheStore
{
    ...
    public function foundryt(string $key): void;
...
foundry inspect feature publish_post
foundry inspect route POST /posts
foundry inspect auth publish_post
foundry inspect cache publish_post
foundry inspect events publish_post
foundry inspect jobs publish_post
foundry inspect context publish_post
foundry inspect dependencies publish_post

Generation

foundry generate feature specs/publish_post.yaml
foundry generate indexes
foundry generate tests publish_post
foundry generate migration specs/add_posts_table.yaml
foundry generate context publish_post

Verification

foundry verify feature publish_post
foundry verify contracts
foundry verify auth
foundry verify cache
foundry verify events
foundry verify jobs
foundry verify migrations

Runtime

foundry serve
foundry queue:work
foundry queue:inspect
foundry schedule:run
foundry trace:tail

Planning / impact analysis

foundry affected-files publish_post
foundry impacted-features posts.create
foundry impacted-features event:post.created
foundry impacted-features cache:posts:list
...
For foundry inspect feature publish_post --json, use a shape like:
...
foundry generate feature specs/publish_post.yaml must generate:
	•	feature folder
	•	schemas
	•	action stub
	•	query stub
...
Create:
	...
	•	foundry inspect feature
	•	foundry generate feature
	•	foundry generate indexes
	•	foundry verify feature
...
Produce:
	1.	full Foundry framework codebase
...
Build Foundry as a disciplined, explicit, reusable factory for reliable web features.
